...
# ═══════════════════════════════════════════════════════════
# CHART IMAGE FETCHING (Chart-Img API)
# ═══════════════════════════════════════════════════════════

def get_chart_image(pair, interval="D"):
    """Fetch TradingView chart screenshot from Chart-Img API"""
    if not CHART_IMG_KEY:
        logger.warning("⚠️ CHART_IMG_KEY not set - skipping chart image")
        return None
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            return r.content  # Return binary image data
        else:
            logger.error(f"❌ Chart API error: {r.status_code} - {r.text}")
...
# ═══════════════════════════════════════════════════════════
# TELEGRAM MESSAGE SENDING (with Chart Images)
# ═══════════════════════════════════════════════════════════

async def send_to_telegram(text, image_data=None, pair="XAUUSD"):
    """Send message with optional chart image to Saved Messages or Vantage group"""
    global client
    ...
    try:
        ...
        if image_data:
            ...
            chart_file = BytesIO(image_data)
            chart_file.name = f"{pair}_chart.png"
            ...
            logger.info(f"✅ Chart image + message sent to {'Saved Messages' if SEND_TO_SAVED else 'Vantage group'}")
        else:
            # Send text-only message (fallback if chart unavailable)
            ...
            logger.info(f"✅ Message sent to {'Saved Messages' if SEND_TO_SAVED else 'Vantage group'}")
...
def calculate_pips(pair, direction, entry_price, current_price):
    ...
    try:
        if pair == "XAUUSD":
            ...
            # Generic Forex: 4 decimal places
            pips = round((current_price - entry_price) * 10000)
        
        if direction == "SELL":
            pips = -pips
        
        return max(0, pips)  # Return only positive pips
    except Exception as e:
...
def mt5_close():
    ...
    try:
        ...
        # Fetch chart image
        chart_image = get_chart_image(pair, interval="D")
        
        # Send message with or without chart
        future = asyncio.run_coroutine_threadsafe(
            send_to_telegram(text, image_data=chart_image, pair=pair),
            loop
        )
        try:
...
def monitor_profits():
    """Monitor open trades and send profit level messages with charts"""
            ...
            for trade_id, trade in trades_copy.items():
                ...
                for level_pips in levels_to_check:
                    if pips >= level_pips and level_pips not in reported_levels.get(trade_id, set()):
                        ...
                        # Fetch chart image
                        logger.info(f"📸 Fetching chart for {pair}...")
                        chart_image = get_chart_image(pair, interval="D")
...
def health():
    ...
    return (
        f"<pre>✅ Trade Alert Bot v2 Running!\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"Mode: {mode}\n"
        f"Active Trades: {active_count}\n"
        f"Telegram: {client_status}\n"
        f"OANDA: {oanda_status}\n"
        f"Timestamp: {datetime.now().isoformat()}</pre>"
    ), 200
